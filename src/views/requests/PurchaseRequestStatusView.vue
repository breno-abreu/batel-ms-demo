  ...
  <section class="repertoire-page">
    <header class="repertoire-page__header repertoire-page__header--actions">
      <div>
        <div class="repertoire-page__header-title">
          ...
          <h1>Status de Compra</h1>
        </div>
        <p class="description repertoire-page__header-description">
          Cadastre e consulte os status usados nas solicitações de compra.
          Cada status deve estar associado a uma das cinco flags gerais.
        </p>
      </div>
    ...
    <section class="repertoire-card">
      ...
      <p v-else-if="items.length === 0" class="description">
        Nenhum status de solicitação de compra cadastrado.
      </p>
        ...
        <ul class="sortable-list sortable-list--draggable" role="list">
          ...
          >
            ...
            <div class="sortable-list__actions">
              ...
              <AppTooltip
                :text="item.linkedPurchaseRequestCount > 0
                  ? 'Não é possível excluir um status vinculado a solicitações de compra'
                  : 'Excluir status'"
              >
                <button
                  type="button"
                  ...
                  :disabled="actionLoadingId === item.id
                    || reorderLoading
                    || item.linkedPurchaseRequestCount > 0"
                  :aria-label="item.linkedPurchaseRequestCount > 0
                    ? 'Exclusão indisponível: status vinculado a solicitações'
                    : 'Excluir status'"
    ...
    >
      <form v-unsaved-changes class="repertoire-form" novalidate @submit.prevent="handleCreateSubmit">
        <label>
          Nome *
          <input
            id="purchase-request-status-name"
            v-model="form.name"
            type="text"
          ...
          >
          <span
            id="purchase-request-status-name-counter"
            class="field-counter"
            aria-live="polite"
          >
          ...
          <span
            v-if="nameError"
            id="purchase-request-status-name-error"
            class="field-error"
            role="alert"
          >
            {{ nameError }}
    ...
    >
      <form v-unsaved-changes class="repertoire-form" novalidate @submit.prevent="handleEditSubmit">
        <label>
          Nome *
          <input
            id="purchase-request-status-edit-name"
...
import {
  buildPurchaseRequestStatusLifecyclePayload,
  getPurchaseRequestStatusLifecycleClass,
  getPurchaseRequestStatusLifecycleFlag,
  getPurchaseRequestStatusLifecycleLabel,
  PURCHASE_REQUEST_STATUS_LIFECYCLE_OPTIONS
} from '@/utils/purchaseRequestStatusLifecycle'

const STATUS_NAME_MAX_LENGTH = 25

const LIFECYCLE_OPTION_ICONS: Record<PurchaseRequestStatusLifecycleFlag, Component> = {
  pending: CircleDotIcon,
  in_analysis: SearchIcon,
  approved: CircleCheckIcon,
  purchased: PackageCheckIcon,
  rejected: CircleXIcon
}

const LIFECYCLE_OPTIONS_WITH_ICONS = PURCHASE_REQUEST_STATUS_LIFECYCLE_OPTIONS.map((option) => ({
  ...option,
...
type StatusForm = {
  name: string
  lifecycleFlag: PurchaseRequestStatusLifecycleFlag
}

export default defineComponent({
  name: 'PurchaseRequestStatusView',
  components: {
    AppFormModal,
    AppListSkeleton,
    AppTooltip,
  ...
  data() {
    return {
      form: {
        name: '',
        lifecycleFlag: 'pending' as PurchaseRequestStatusLifecycleFlag
      } as StatusForm,
      editForm: {
        name: '',
        lifecycleFlag: 'pending' as PurchaseRequestStatusLifecycleFlag
      ...
      items: [] as PurchaseRequestStatus[],
      listLoading: false,
  ...
  methods: {
    getLifecycleLabel(item: PurchaseRequestStatus): string {
      return getPurchaseRequestStatusLifecycleLabel(getPurchaseRequestStatusLifecycleFlag(item))
    },

    getLifecycleClass(item: PurchaseRequestStatus): string {
      return getPurchaseRequestStatusLifecycleClass(getPurchaseRequestStatusLifecycleFlag(item))
    },
    ...
    async loadItems(): Promise<void> {
      ...
      try {
        const response = await purchaseRequestStatusService.list()
        this.items = response.payload ?? []
      } catch (error) {
        toastService.error(
          error instanceof Error ? error.message : 'Erro ao carregar os status de solicitação de compra.'
        )
      } finally {
    ...
    async handleCreateSubmit(): Promise<void> {
      ...
      const payload: CreatePurchaseRequestStatusRequest = {
        name: trimmedName,
        ...buildPurchaseRequestStatusLifecyclePayload(this.form.lifecycleFlag)
      }

      try {
        const response = await purchaseRequestStatusService.create(payload)
        toastService.success(response.message)
        this.closeCreateModal()
        await this.loadItems()
      } catch (error) {
        toastService.error(
          error instanceof Error ? error.message : 'Erro ao cadastrar o status de solicitação de compra.'
        )
      } finally {
    ...
    openEditModal(item: PurchaseRequestStatus): void {
      this.editingItemId = item.id
      this.editForm = {
        name: item.name,
        lifecycleFlag: getPurchaseRequestStatusLifecycleFlag(item)
      }
      this.editNameError = ''
    ...
    async handleEditSubmit(): Promise<void> {
      ...
      const payload: UpdatePurchaseRequestStatusRequest = {
        name: trimmedName,
        ...buildPurchaseRequestStatusLifecyclePayload(this.editForm.lifecycleFlag)
      }

      try {
        const response = await purchaseRequestStatusService.update(this.editingItemId, payload)
        ...
        toastService.error(
          error instanceof Error ? error.message : 'Erro ao atualizar o status de solicitação de compra.'
        )
      } finally {
    ...
    async persistOrder(
      items: PurchaseRequestStatus[],
      previousItems: PurchaseRequestStatus[]
    ): Promise<void> {
      this.reorderLoading = true

      try {
        const response = await purchaseRequestStatusService.reorder({
    ...
    async handleDelete(item: PurchaseRequestStatus): Promise<void> {
      if (item.linkedPurchaseRequestCount > 0) {
        return
      }

      const confirmed = await confirmDialogService.confirm({
        title: 'Excluir status de compra',
      ...
      try {
        const response = await purchaseRequestStatusService.remove(item.id)
        this.items = this.items.filter((currentItem) => currentItem.id !== item.id)
        toastService.success(response.message)
      } catch (error) {
        toastService.error(
          error instanceof Error ? error.message : 'Erro ao excluir o status de solicitação de compra.'
        )
      } finally {
